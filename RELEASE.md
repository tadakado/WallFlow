...
現在の `v1.0.0` は Developer ID 公証済みリリースとして公開する想定です。
...
公証用の認証情報は、事前に Keychain に保存しておきます。`TEAM_ID` は Apple Developer の Team ID、パスワードは Apple ID の通常パスワードではなく App 用パスワードです。

```sh
xcrun notarytool store-credentials "WallFlow-notary" \
  --apple-id "tadakado@gmail.com" \
  --team-id "TEAM_ID" \
...
```sh
hdiutil verify dist/WallFlow-1.0.0.dmg
shasum -a 256 dist/WallFlow-1.0.0.dmg
```
...
```sh
TEAM_ID="TEAM_ID" \
NOTARY_PROFILE="WallFlow-notary" \
...
1. Release ビルドを Developer ID 署名で作成
2. アプリの署名を検証
3. DMG を作成
4. DMG を Developer ID 署名
5. Apple の notary service に送信
6. 公証チケットを DMG に staple
7. `spctl` と `stapler` で検証
8. SHA-256 を `dist/WallFlow-<version>.dmg.sha256` に出力

Developer ID 証明書や公証用認証情報がまだない状態で DMG だけを作る場合は、従来どおり `scripts/make-dmg.sh` を使います。
...
```sh
xcrun notarytool submit dist/WallFlow-1.0.0.dmg --keychain-profile "WallFlow-notary" --wait
xcrun stapler staple dist/WallFlow-1.0.0.dmg
spctl -a -t open --context context:primary-signature -v dist/WallFlow-1.0.0.dmg
```

## GitHub に公開する

`WallFlow/AnimeCharacterClassifier.mlmodelc/` はサイズが大きいため Git にはコミットしません。
リリース前に `./scripts/prepare_anime_character_classifier.sh` で生成し、配布用 DMG に同梱します。
公開時は GitHub Release に DMG をアップロードすることで、利用者へモデル込みのアプリを配布します。
...
```sh
gh release create v1.0.0 dist/WallFlow-1.0.0.dmg \
  --target main \
  --title "WallFlow 1.0.0" \
  --notes "WallFlow 1.0.0 公開リリース。"
```

既存の Release にある DMG を差し替える場合:

```sh
gh release upload v1.0.0 dist/WallFlow-1.0.0.dmg --clobber
```

Release の内容を確認します。

```sh
gh release view v1.0.0 --json tagName,name,url,assets
```
